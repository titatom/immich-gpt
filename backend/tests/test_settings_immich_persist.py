...
def test_save_immich_settings_connection_failure(client):
    ...
    # POST saves settings even on failure, but returns connected=False
    assert r.status_code == 200
...
def test_get_immich_settings_reads_from_db(client, db):
    ...
    db.add(AppSetting(key="immich_url", value="http://db.local"))
    db.add(AppSetting(key="immich_api_key", value="db-key"))
    db.commit()
